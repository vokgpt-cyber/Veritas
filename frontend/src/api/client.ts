...
export async function getDeveloperSettings(): Promise<DeveloperSettings> {
  return request<DeveloperSettings>("/developer/settings");
}

export async function updateDeveloperActive(payload: Partial<DeveloperSettings["active"]>): Promise<DeveloperSettings> {
  return request<DeveloperSettings>("/developer/settings/active", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function updateDeveloperPrompt(
  promptId: string,
  payload: Pick<DeveloperPrompt, "enabled" | "system_template" | "user_template" | "notes">,
): Promise<DeveloperPrompt> {
  return request<DeveloperPrompt>(
    `/developer/prompts/${encodeURIComponent(promptId)}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
...
export function getDeveloperTechnicalLogUrl(jobId: string): string {
  return `${API_BASE}/developer/meetings/${encodeURIComponent(jobId)}/technical-log`;
}

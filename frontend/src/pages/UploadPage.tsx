...
const UPLOAD_COPY = {
  en: {
    ...
    adminAttendeesHelp:
      "Select attendees; their names are added to the summarization context and help map SPEAKER_XX to real names.",
  ...
  ru: {
    ...
    adminAttendeesHelp:
      "Отметьте присутствующих: эти имена попадут в контекст саммаризации и помогут сопоставить SPEAKER_XX с реальными людьми.",
    ...
    try {
      let speakerRange =
        meetingType === "administrative"
          ? undefined
          : resolveSpeakerBucket(speakerBucket) ?? undefined;
      if (meetingType === "administrative") {
        const adminSpeakerCount =
          getSelectedAdminProfiles().length + parseCustomAttendees().length;
        if (adminSpeakerCount > 0) {
          speakerRange = {
            min: Math.max(1, adminSpeakerCount - 2),
            max: Math.min(40, adminSpeakerCount + 2),
          };
        }
      }
        ...
        <div className="mt-6 space-y-5">
          ...
          {meetingType === "administrative" && (
            <div>
              ...
              <p className="mt-1 text-xs text-epam-gray-400">
                {language === "ru"
                  ? "Список по умолчанию настраивается в разделе \"Голоса\". Число спикеров для pyannote будет рассчитано по выбранным участникам."
                  : "The default list is configured in Voices. pyannote speaker count is derived from selected attendees."}
              </p>
            </div>
          )}
          ...
          {meetingType !== "court_hearing" && meetingType !== "administrative" && (
          <div>
            <label

@echo off
REM A/B ASR comparison on the Admin audio (ASCII-only bat, Cyrillic in helper).
setlocal enableextensions
cd /d "%~dp0.."
set LOGFILE=scripts\run_ab_asr_log.txt

echo ============================================================ > "%LOGFILE%"
echo A/B ASR run - %DATE% %TIME% >> "%LOGFILE%"
...
if not exist "scripts\run_ab_asr_helper.py" (
    echo ERROR: helper not found at scripts\run_ab_asr_helper.py
...
echo [launcher] Starting A/B ASR run. Expect ~2-5 minutes.
echo [launcher] Progress is written to %LOGFILE%.
echo.

venv\Scripts\python.exe -u scripts\run_ab_asr_helper.py >> "%LOGFILE%" 2>&1
